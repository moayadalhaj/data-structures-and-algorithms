...
function transformToLis(obj) {
  ...
  let newArray = objToArr.map(element => `<li>${element[0]}: ${element[1]}</li>`);
  return newArray;
}
...
const count = (target, input) => {
  // Solution code here...
  let count = 0;
  input.map(element => element.map(el => el === target ? count++ : count));
  return count;
};
...
const totalSum = (input) => {
  // Solution code here...
  let count = 0;
  input.map(element => element.map(el => count += el));
  return count;
};
  ...
  input.map((item, index) => {
    ...
    item.map(value => {
      if (typeof (value) === 'number' && value % 5 == 0) {
        newArray[index].push(Math.pow(2, value));
      }
    });
  });
  return newArray;
...
let findShortest = (data) => {
  // Solution code here...
  let newArray = data.map(element => element)
    .reduce((acc, value) => acc.height > value.height ? acc : value);
