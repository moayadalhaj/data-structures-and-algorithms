...
const screenForNames = (arr) => {
  ...
  let newArray = arr.filter(element => element.match(regEx));
  return newArray;
}
...
const detectTicTacToeWin = (board) => {
  ...
  let result;
  function helper(arr) {
    let newArray1 = arr.every(element => element === 'X');
    let newArray2 = arr.every(element => element === 'O');
  ...
  for (let i = 0; i < board.length; i++) {
    ...
    if (helper(row1) || helper(col1) || helper(diag1) || helper(diag2)) {
      result = true;
      break;
    } else {
      result = false;
